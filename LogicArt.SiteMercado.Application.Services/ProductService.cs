...
namespace LogicArt.SiteMercado.Application.Services
{
    public class ProductService : Service<Product, Guid>, IProductService
    {
        ...
        {
            var products = await this.Repository.Query().OrderBy(x => x.Name).ToListAsync();
            return products;
        }

        public async Task<IEnumerable<Product>> GetProductsByNameAsync(string name)
        {
            var products = await this.Repository.Query().Where(x => x.Name.Contains(name)).OrderBy(x => x.Name).ToListAsync();
            return products;
        }
    }
}
